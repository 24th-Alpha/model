...
from  .VTO.gradio_demo.app import start_tryon
...
def try_on(request):
    if request.method == 'POST':
        ...
        return JsonResponse({'try_on_result': try_on_result['result_path']})

    return HttpResponseBadRequest("Invalid request method")


def save_base64_to_file(base64_string, extension):
    # Convert base64 string to a temporary image file
    image_data = base64.b64decode(base64_string)
    temp_file = BytesIO(image_data)
    temp_image = Image.open(temp_file)
    file_path = f"temp_image{extension}"
...
def generate_try_on_result(cloth_image, person_image, category):
    ...
    try:
        ...
        output_image, mask_gray = start_tryon(dict_input, garm_img, garment_des, is_checked, is_checked_crop, denoise_steps, seed)
